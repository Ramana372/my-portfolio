...
const ResumeHeader = styled.div`
  text-align: ${props => props.type === 'general' ? 'left' : 'center'};
  margin-bottom: 30px;
  color: #fff;
  padding: ${props => props.type === 'general' ? '0 20px' : '0'};
...
const CVPopup = ({ isOpen, onClose, type }) => {
  if (!isOpen) return null;

  const handleDownload = () => {
    const link = document.createElement('a');
    link.href = type === 'general' 
      ? '/assets/general-cv.pdf' 
      : '/assets/specialized-cv.pdf';
    link.download = `Venkata_Ramana_${type === 'general' ? 'General' : 'Specialized'}_CV.pdf`;
      ...
      <PopupContent onClick={e => e.stopPropagation()}>
        ...
        <div className="resume">
          <DownloadButton onClick={handleDownload}>
            ...
            Download {type === 'general' ? 'General' : 'Specialized'} CV
          </DownloadButton>
          <ResumeHeader type={type}>
          ...
          <ResumeSection>
            <h3>SKILLS</h3>
            <p><strong>Languages:</strong> Java, C++, DSA, SQL, JavaScript, PHP</p>
            <p><strong>Frameworks:</strong> HTML and CSS, React, Node.js, Express, MongoDB</p>
            <p><strong>Developer Tools:</strong> Git, GitHub, AWS, VS Code, MySQL, MongoDB, Docker, Jenkins, Ubuntu</p>
            <p><strong>Soft Skills:</strong> Problem-Solving Skills, Team Player, Project Management, Responsive Web Design</p>
          </ResumeSection>  
          <ResumeSection>
            <h3>PROJECTS</h3>
            <ResumeItem>
          ...
          <ResumeSection>
            <h3>CERTIFICATES/CERTIFICATION</h3>
            <ResumeItem>
